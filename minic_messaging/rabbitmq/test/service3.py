from minic_messaging.rabbitmq.adapter import RabbitmqBlockingExchangeAdapter, RabbitmqBlockingTaskQueueAdapter
import time

if __name__ == "__main__":
    adapter1 = RabbitmqBlockingExchangeAdapter()
    adapter1.subscribe("service1", "minic31", "minic13", "topic", ["#"])
    adapter1.subscribe("service2", "minic32", "minic23", "topic", ["#"])

    adapter2 = RabbitmqBlockingTaskQueueAdapter()
    adapter2.subscribe("task-queue")

    
    while True:
        time.sleep(1)
        adapter1.publish("service1", "minic31", "#", "hello from 3")
        adapter1.publish("service2", "minic32", "#", "hello from 3")